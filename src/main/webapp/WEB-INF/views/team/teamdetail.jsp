...
<div id="jb-container">
	...
	<div id="jb-content1">
		...
		<div class="card mb-3">
			<h3 class="card-header">팀명:${teamdetail.team_name }</h3>
			<div class="card-body">
				<h5 class="card-title">팀장:${teamdetail.leader }</h5>
				<h6 class="card-subtitle text-muted"></h6>
			</div>
			<c:choose>
				<c:when test="${empty teamdetail.file_name }">
					<img alt="qqwe" src="/image/team.png"
			...
			<div class="card-body">
				<p class="card-text">${teamdetail.title }</p>
			</div>
			<ul class="list-group list-group-flush" id="_list_group">
				<li class="list-group-item">
					<h4>창단일</h4>
					<p><fmt:formatDate value="${teamdetail.fdate }" pattern="yyyy-MM-dd" /></p>
				</li>
				<li class="list-group-item">
					<h4>팀원수</h4>
					<p>${memcount }명</p>
	...
	<div id="jb-content2">
		...
		<ul class="nav nav-tabs">
			<li class="active">
				<a class="nav-link" data-toggle="tab" href="#profile">팀내용</a>
			</li>
			<li class="nav-item">
				<a class="nav-link" data-toggle="tab" href="#member">팀원</a>
			</li>
			<li class="nav-item">
				<a class="nav-link" data-toggle="tab" href="#mmr">MMR</a>
			</li>
			<li class="nav-item">
		...
		<div id="myTabContent" class="tab-content">
				...
				<table class="table table-hover table-responsive">
					<thead>
						<tr>
							...
							<th>키</th><th>몸무게</th><th>직책</th>
						</tr>
					</thead>
					
				<c:forEach items="${memberlist }" var="member" begin="0" end="9" step="1" varStatus="vs">
					<tbody>
						<tr>
							<td>${vs.count }</td><td>${member.id }</td><td>${member.name }</td>
							<td>${member.height }</td><td>${member.weight }</td>
							<td>
							<c:if test="${teamdetail.leader eq member.id }">팀장</c:if> 
							<c:if test="${teamdetail.leader ne member.id }">팀원</c:if> 
			...
			<div class="tab-pane fade" id="mmr">
				<table class="table table-hover table-responsive">
					...
					<tbody>
						<tr>
							<td>축구</td><td>${teamdetail.soccer_mmr }</td>
						</tr>
						<tr>
							<td>농구</td><td>${teamdetail.basketball_mmr }</td>
						</tr>
						<tr>
							<td>야구</td><td>${teamdetail.baseball_mmr }</td>
						</tr>
						<tr>
							<td>배드민턴</td><td>${teamdetail.badminton_mmr }</td>
						</tr>
						<tr>
							<td>탁구</td><td>${teamdetail.pingpong_mmr }</td>
						</tr>
					</tbody>
				</table>
			</div>
	...
	<div id="jb-sidebar">
		...
		<div class="list-group">
			<c:choose>
				...
				<c:otherwise>
					<c:if test="${teamappcheck > 0 }">
						<a class="list-group-item list-group-item-action disable">신청한 팀입니다.</a>
					</c:if>
				</c:otherwise>
			</c:choose>
			...
			<c:choose>
				<c:when test="${mem.team_name eq teamdetail.team_name && auth.authority eq 'ROLE_TEAMLEADER' }">
				...
				<a href="#" class="list-group-item list-group-item-action" id="_teamup">팀상세정보 수정</a>
					...
					<input type="hidden" id="_file_name" value="${teamdetail.file_name }">
				<a href="matchinglist.do?name_team1=${teamdetail.team_name }&check=1"
					class="list-group-item list-group-item-action"
				...
				<a class="list-group-item list-group-item-action disabled">팀상세정보 수정</a>
				
				<a href="matchinglist.do?name_team1=${teamdetail.team_name }&check=0"
					class="list-group-item list-group-item-action"
					data-toggle="tooltip" data-placement="left" title=""
				...
				<c:choose>
					<c:when test="${mem.team_name eq teamdetail.team_name && auth.authority eq 'ROLE_TEAMMEMBER' }">
						<a href="teamSecession.do?id=${user.username }" class="list-group-item list-group-item-action">팀탈퇴</a>
					</c:when>
					<c:otherwise>
						<a href="#" class="list-group-item list-group-item-action disabled">팀탈퇴</a>
					</c:otherwise>
				</c:choose>	
...
<script type="text/javascript">
	$(document).ready(function name() {
		...
		$(document).on("click", "#_teamup", function () {
			location.href="detailup.do?team_name=" + team_name + "&sport_category=" + sport_category +
