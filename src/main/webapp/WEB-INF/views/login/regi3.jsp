 ...
 <form id="regiFrm">
	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
  	<table>
  		...
  		<tr>
  			<td>
  				<input type="text" placeholder="id 입력" id="id" name="id" onke<input type="text" placeholder="id 입력" id="id" name="id" onkeydown="fn_press_han(this);" style="ime-mode:disabled;">
  				<button type="button" class="btn btn-default" id="idcheckbtn">id체크</button>
  				<input type="hidden" id="idsave" ><br>
				<font color="red" id="idfont">7 ~ 20글자의 영문+숫자로 등록해주십시오</font>>
  		</tr>
  		<tr>
  			<td>
  				<input type="password" placeholder="비밀번호 입력" id="pwd" name="pwpwd
  			</td>
  		</tr><font color="red" id="pwdfont">8~15자리의 특수,영문+숫자로 등록해주십시오
				( ~!@\#$%&lt;&gt;^&amp;* 만 지원합니다. )</font>mp;* 만 지원합니다. )</font>
  			</td>
  		</tr>
  		<tr>
  			<td>
  			...
  			="button" class="btn btn-default" id="addressbtn"<input type="text" placeholder="생년월일(예 : 19920305)" id="birthday" name="birthday" maxlength="8"
  				 onkeypress="return fn_press(event,'numbers');" onkeydown="fn_press_han(this);" style="ime-mode:disabled;">" placeholder="생년월일(예 : 19920305)" id="birthday" <input type="text" placeholder="전화번호(예 : 01012345678)" id="phone" name="phone" maxlength="11"
  				 onkeypress="return fn_press(event,'numbers');" onkeydown="fn_press_han(this);" style="ime-mode:disabled;">"opacity: 0">공간지정</span>
  			</td>
  		</tr><input type="text" placeholder="이메일(예 : abc@naver.com)" id="email" name="email" onkeydown="fn_press_han(this);" style="ime-mode:disabled;">
  				 <input type="button" value="인증번호 전송" class="btn btn-default" id="cernumbtn">:disabled;">
  			</td>
  		</tr>
  		<tr>
  			<td>
...
<input type="text" placeholder="신장(예 : 165.3)" id="height" name="height" maxlength="5"
  				  onkeydown="fn_press_han(this);" style="ime-mode:disabled;">cmput type="button" value="인증번호 전송" class="btn bt<input type="text" placeholder="체중(예 : 63.4)" id="weight" name="weight" maxlength="4"
  				 onkeydown="fn_press_han(this);" style="ime-mode:disabled;">kg		<tr>
  			<td>
	...
	<br>
	 ...
	 0">공간지정</span>
  		...
  		</tr class="btn btn-info">원가입				<inpu
	<button type="button" id="loginBtn" class="btn btn-defalut">로그인
		하러가기</button>
	...
	$(function() {
							...
							if ($("#email").val().trim().indexOf("@") == -1) {
								...
								$
										.ajax({
											...
											data : "email="
													+ $("#email").val().trim()
													+ "&cerNumber=" + number,//Parameter(넘겨주는 값)
											////////////////////////////////////////////////////
		...
		$("#id").on(
				...
				function() {
					...
					if (id.length >= 7 && id.length <= 20 && pattern1.test(id)
							&& pattern2.test(id) && !pattern3.test(id)) {
		...
		$("#pwd").on(
				...
				function() {
					...
					if (pwd.length >= 8 && pwd.length <= 15
							&& pattern1.test(pwd) && pattern2.test(pwd)
							&& pattern3.test(pwd)) {
				...
				.click(
						function() {
							...
							if (check == false) {
								...
								if (confirm("입력하신 " + $("#idsave").val()
										+ " 로 가입하게 됩니다 가입하시겠습니까?") == true) {
		...
		$("#idcheckbtn").click(
				function() {
					...
					if ($("#id").val().trim().length < 7
							|| !pattern1.test($("#id").val().trim())) {
